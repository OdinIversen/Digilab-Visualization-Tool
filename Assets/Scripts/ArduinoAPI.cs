...
public class ArduinoAPI : SensorAPIBase
{
    private string ClientId = Environment.GetEnvironmentVariable("DIGILAB_ARDUINO_CLIENT_ID",EnvironmentVariableTarget.User);
    private string ClientSecret = Environment.GetEnvironmentVariable("DIGILAB_ARDUINO_CLIENT_SECRET",EnvironmentVariableTarget.User);
    ...
    private List<Sensor> ConvertDevicesToSensorData()
    {
        ...
        RaiseOnSensorDataUpdated(sensorData);  // Use the method to raise the event
